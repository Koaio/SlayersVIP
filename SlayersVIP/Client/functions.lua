...
function SpawnPed(pedname)
    ...
    isWPed = true
    if Koaio.particule and HasModelLoaded(p1) then
        ParticleMaker("scr_rcbarry2", "scr_clown_appears", 0.4) Citizen.Wait(100)
    end
    Advancednotif('Menu VIP', '~r~Notification~s~', 'Voici le ped que vous avez choisi !~s~', 'CHAR_SLAYERSRP', 1)
    -- Anti Spam
...
function Timeout()
    ...
    ESX.ShowNotification("🛡 ~g~En tant que VIP vous avez le droit d'utiliser qu'un seul ped toutes les 30 minutes")
    Wait(1800000)
    PedTimeout = true
    ...
    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
        ...
        TriggerEvent('skinchanger:loadDefaultModel', isMale, function()
            ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
                ...
                TriggerEvent('esx:restoreLoadout')
                isWPed = false
                Advancednotif('Menu VIP', '~r~Notification~s~', 'Votre personnage a bien été réinitialisé !~s~', 'CHAR_SLAYERSRP', 1)
            end)
        end)
    end)
end
